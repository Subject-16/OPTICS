...
ll      = -10:0.2:10; % number of OAM modes needed
n       = 0;    % roughness index
thr     = 6e-3;
...
for ii = 1:length(ll)
    ...
    for jj = 1:length(a)
        if (abs(a(jj))< 1e-3 && abs(a(jj))>0.5e-3)
...
LL      = ll;
thr2    = 9e-6;% threshold 2nd order coherence peaks 
...
%     %%%%%%%%%%%%%%%%%%%%%%%
    ...
    for jj = 1:length(a)
        if (abs(a(jj))< 1e-3 && abs(a(jj))>0.49e-3)
...
Theo_corr2 = (alpha1/pi)^4*((sin(alpha1.*ll/2)./(alpha1.*ll/2)).^4)...
    .*(cos(phi1*ll*2*3/2))/(2*pi^6);

figure
plot(LL,LL_value,'-*')
title('Second second order Intensity')    
hold
plot(LL,(Theo_corr2)/1.7*10^4 + 10^(-4),'-')
