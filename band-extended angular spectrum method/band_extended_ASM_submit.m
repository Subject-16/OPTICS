...
clear all
close all
%% object parameters and propagation parameters | creating rectangular aperture
n0 = 1024;                                    % number of the original signal
r = n0/4;                                     % number of the rectangle aperture
t = zeros(n0,1);
t (n0/2-r+1:n0/2+r,1) = 1;
t = padarray(t,n0/2);                         % zero-padded signal(both sides) (input to padarray must be column array
lam = 500e-9;                                 % wavelength
k = 2*pi/lam;                                 % wave number
n = size(t,1);                                % number of the zero-padded signal
pitch = 1e-3;                                % sampling pitch in spatial domain
l = n*pitch;                  
